	...
	BE::State: StateBackend<BlakeTwo256>,
{
	...
	pub fn max_priority_fee_per_gas(&self) -> Result<U256> {
		// https://github.com/ethereum/go-ethereum/blob/polkadot-v0.9.33/eth/ethconfig/config.go#L44-L51
		...
		// https://github.com/ethereum/go-ethereum/blob/polkadot-v0.9.33/eth/gasprice/gasprice.go#L149
		let mut rewards = Vec::new();
		if let Ok(fee_history_cache) = &self.fee_history_cache.lock() {
			for n in lowest..highest + 1 {
				if let Some(block) = fee_history_cache.get(&n) {
